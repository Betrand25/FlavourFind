    ...
    window.redeemCoupon = function() {
        if (!loggedInEmail) {
            ...
            if (!hasReviewed) {
                ...
                if (redeemedCoupons[loggedInEmail] && redeemedCoupons[loggedInEmail]['NEWMEMBER']) {
                    alert('You have already redeemed this coupon. It can only be used once.');
                } else {
                    const couponCode = generateCouponCode('NEWMEMBER');
                    ...
                    redeemedCoupons[loggedInEmail]['NEWMEMBER'] = true;
