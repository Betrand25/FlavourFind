document.addEventListener('DOMContentLoaded', () => {
    ...
    window.redeemCoupon = function() {
        if (!loggedInEmail) {
            ...
            // Check if the ramen coupon has already been redeemed
            const redeemedCoupons = JSON.parse(localStorage.getItem('redeemedCoupons')) || {};
            if (redeemedCoupons[loggedInEmail] && redeemedCoupons[loggedInEmail]['RAMEN']) {
                alert('You have already redeemed this coupon. It can only be used once.');
            } else {
                const couponCode = generateCouponCode('RAMEN');
                ...
                redeemedCoupons[loggedInEmail]['RAMEN'] = true;
